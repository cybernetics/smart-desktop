...
final class ButtonMustBeSelectedModel implements Validator<ButtonModel[]> {
    ...
    ButtonMustBeSelectedModel (String message) {
        this.message = message;
    }

    public boolean validate(Problems problems, String compName, ButtonModel[] model) {
        boolean sel = false;
        for (ButtonModel m : model) {
            sel = m.isSelected();
            if (sel) break;
        }
        if (!sel) {
            problems.add(message, Severity.FATAL);
            return false;
        }
        return true;
    }
