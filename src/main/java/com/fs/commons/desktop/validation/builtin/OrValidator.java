...
final class OrValidator<T> implements Validator<T> {
    ...
    public OrValidator(Validator<T>... initial) {
    ...
    public boolean validate(Problems problems, String compName, T model) {
        ...
        for (Validator v : validators) {
            boolean nextResult = v.validate(problems, compName, model);
            result &= nextResult;
        }
        return result;
    }

    public void or(Validator<T> v) {
        orImpl (v);
    }

    Validator<T> orImpl(Validator<T> v) {
        validators.add (0, v);
        return this;
    }
    ...
    public String toString() {
        return "OrValidator for " + validators;
    }
}
